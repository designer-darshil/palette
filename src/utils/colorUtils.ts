...
export function getContrastRating(ratio: number): { label: string; passAA: boolean; passAAA: boolean } {
  return {
    label: ratio >= 7 ? 'AAA Pass' : ratio >= 4.5 ? 'AA Pass' : ratio >= 3 ? 'Large Text Only' : 'Fail',
    passAA: ratio >= 4.5,
    passAAA: ratio >= 7,
  };
}
